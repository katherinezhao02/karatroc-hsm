...
void sha256_digest(struct sha256 *sha256, const uint8_t *buf, size_t n, uint8_t *digest) {
    ...
    uint32_t ctrl_go = SHA256_CTRL_INIT;
    ...
    while (n >= SHA256_BLOCK_SIZE) {
        // wait until ready
        while (!(sha256->status & SHA256_STATUS_READY)) {
            // wait
        }
        // fill data
        for (i = 0; i < SHA256_BLOCK_SIZE/4; i++) {
            sha256->blocks[i] = swap32(*((const uint32_t *) buf));
            buf += 4;
        }
        // issue write
        sha256->ctrl = ctrl_go;
        ctrl_go = SHA256_CTRL_NEXT;
        n -= SHA256_BLOCK_SIZE;
    }
    ...
    // write size
    sha256->blocks[i++] = swap32(*((uint32_t *) &bits64));
    sha256->blocks[i] = swap32(*(((uint32_t *) &bits64) + 1));
    ...
    for (i = 0; i < SHA256_DIGEST_SIZE/4; i++) {
        digest_w[i] = swap32(sha256->digest[i]);
    }
}
