...
(define (wait-until-start-bit)
  ...
  (if (output-tx (in))
      (begin
        ;; all sha256-related hints here, this is where we wait for main work to be done
        (hint initial-inject)
        (hint pre-tick)
        (tick)
        (hint post-tick)
...
(define (wait-and-return value)
  (tick-n 80) ; to give circuit time to power off
  (hint finalize)
  value)

(define (wait-trng-req)
